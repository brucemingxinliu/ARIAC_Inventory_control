...
public:
	NewActionServer::newActionServer();
...
void NewActionServer::do_spin(double spin_ang) {
    ros::Rate loop_timer(1/g_sample_dt);
    double timer=0.0;
    double final_time = fabs(spin_ang)/g_spin_speed;
    g_twist_cmd.angular.z= 1.0*g_spin_speed;
...
void newActionServer::do_move(double distance) { // always assumes robot is already oriented properly
    ...
    g_twist_cmd.linear.x = 1.0*g_move_speed;
    while(timer<final_time) {
          g_twist_commander.publish(g_twist_cmd);
          timer+=g_sample_dt;
          loop_timer.sleep(); 
